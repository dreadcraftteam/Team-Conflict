...
ConVar coplay_joinfilter("p2p_joinfilter", "2", FCVAR_ARCHIVE, "Whos allowed to connect to our Game? Will also call p2p_opensocket on server start if set above -1.\n"
                       "-1 : Off\n"
                       "0  : Controlled\n"
                       "1  : Friends Only\n"
                       "2  : Anyone\n",
...
void CCoplayHost::StartHosting()
{
    ...
    {
        ConColorMsg(COPLAY_MSG_COLOR, "[P2P] You're not currently in a local game.");
        return;
    }
    ...
    {
        ConColorMsg(COPLAY_MSG_COLOR, "[P2P] You're not currently in a local game.%s\n", netinfo->GetAddress());
        return;
    }
