...
ConVar coplay_use_lobbies("p2p_use_lobbies", "1", FCVAR_HIDDEN, "Use Steam Lobbies for connections.\n");
#endif
ConVar coplay_debuglog_steamconnstatus("p2p_debuglog_steamconnstatus", "0", 0, "Prints more detailed steam connection statuses.\n");
ConVar coplay_debuglog_lobbyupdated("p2p_debuglog_lobbyupdated", "0", 0, "Prints when a lobby is created, joined or left.\n");
ConVar coplay_autoopen("p2p_autoopen", "1", FCVAR_ARCHIVE, "Open game for listening on local server start");
...
{
    ConColorMsg(COPLAY_MSG_COLOR, "[P2P] Initialization started...\n");

    if (SDL_Init(0))
    {
        Error("[P2P Error] SDL Failed to Initialize: \"%s\"", SDL_GetError());
    }

    if (SDLNet_Init())
    {
        Error("[P2P Error] SDLNet Failed to Initialize: \"%s\"", SDLNet_GetError());
    }
...
void CCoplaySystem::PostInit()
{
	...
	// Executing p2p_connect as soon as we can will probably fail since we're likely not connected to steam networking yet.
	// Save it for later
	int paramIndex = CommandLine()->FindParm("+p2p_connect");
	if (paramIndex)
	{
...
void CCoplaySystem::Update(float frametime)
{
    ...
    if (lastupdated + 1 < gpGlobals->realtime)
    {
        if (GetHost()->IsHosting() && coplay_joinfilter.GetInt() != eP2PFilter_CONTROLLED)
        {
            ...
            SteamFriends()->SetRichPresence("p2p_playercount", std::to_string(GetHost()->GetConnectionCount()+1).c_str());
        }
        else if (GetRole() == eConnectionRole_INACTIVE && engine->IsConnected()) // On normal server
        {
            ...
            SteamFriends()->SetRichPresence("connect",connect.c_str());
            SteamFriends()->SetRichPresence("p2p_playercount", "");
        }
        else
        {
    ...
    {
        m_queuedCommand = "p2p_connect " + m_queuedCommand;
        CCommand args;
...
void CCoplaySystem::JoinGame(GameRichPresenceJoinRequested_t *pParam)
{
	...
	if (command.empty() || (command.find("+connect") != 0 && command.find("+p2p_connect") != 0)
		|| command.find_first_of("\'\"\\/;") != std::string::npos )
	{
		ConColorMsg(COPLAY_DEBUG_MSG_COLOR, "[P2P Warning] Got a bad join string ( %s ) "
...
void CCoplaySystem::CoplayConnect(const CCommand& args)
{
    ...
    if (SteamNetworkingUtils()->GetRelayNetworkStatus(nullptr) != k_ESteamNetworkingAvailability_Current)
    {
        Warning("[P2P Warning] Can't Connect! Connection to Steam Datagram Relay not yet established.\n");
    ...
    if (UseCoplayLobbies() && steamid.IsLobby())
    {
		...
		ConColorMsg(COPLAY_MSG_COLOR, "[P2P] Attempting to join lobby with ID %s....\n", destination.c_str());
    ...
    Warning("P2p_Connect was called with an invalid SteamID! ( %llu )\n", steamid.ConvertToUint64());
}
...
{
    ConColorMsg(COPLAY_MSG_COLOR, "P2P was written based on Coplay (by CoaXioN-Games).\n\n");
    ConColorMsg(COPLAY_MSG_COLOR, "Coplay version: 1.3\n");
    ConColorMsg(COPLAY_MSG_COLOR, "P2P version: 1.0\n\n");
    ConColorMsg(COPLAY_MSG_COLOR, "Coplay: https://github.com/CoaXioN-Games/coplay\n");
    ConColorMsg(COPLAY_MSG_COLOR, "TC-SDK: https://github.com/dreadcraftteam/TEAM-CONFLICT\n\n");
    ConColorMsg(COPLAY_MSG_COLOR, "Good Luck!\n");
}
...
std::string CCoplaySystem::GetConnectCommand()
{
    ...
    if (coplay_joinfilter.GetInt() == eP2PFilter_CONTROLLED && !UseCoplayLobbies())
        cmd = "p2p_connect " + std::to_string(id) + " " + GetHost()->GetPasscode();
    else
        cmd = "p2p_connect " + std::to_string(id);
...
void CCoplaySystem::InvitePlayer(const CCommand& args)
{
    ...
    {
        ConColorMsg(COPLAY_MSG_COLOR, "[P2P] You're not currently hosting a game joinable by P2P.\n");
        return;
    }
    ...
    if (UseCoplayLobbies() && coplay_joinfilter.GetInt() != eP2PFilter_EVERYONE)
    {
        ...
        {
            ConColorMsg(COPLAY_MSG_COLOR, "[P2P] You aren't in a lobby.\n");
            return;
        }
...
void CCoplaySystem::ReRandomizePassword(const CCommand& args)
{
    ...
    {
        ConColorMsg(COPLAY_MSG_COLOR, "[P2P] You're not currently hosting a game.");
        return;
    }
